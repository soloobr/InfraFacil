...
public class Status_Ordem extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    EditText editDescri, DataPrev,  editData;
    ...
    //private TextView tx;
    String nid, nnome, neditDescri, nDataPrev,  neditServRealizado,  neditTec, neditValorP , neditValorM, neditValorTotal, neditStatus, nidcliocor;
    String ncomp;

    private static String url = "http://futsexta.16mb.com/Poker/Get_OrdemM.php";
        ...
        switch (v.getId()) {
            ...
            case R.id.btnfinaliza:
                if ("CANCELAR".matches(btnfinaliza.getText().toString())) {
                    ...
                    alerta.setTitle("Confirmar Pagamento Total ?");
                    alerta.setIcon(R.mipmap.ic_launcher);
                    alerta.setMessage("Confirmar o Pagamento Total ?");
            ...
            if (json != null) {
                try {
                    ...
                    for (int i = 0; i < eventDetails.length(); i++)
                    {
                        ...
                        String idclienteocor = object.getString("ID_Cliente_Ocor");
                        String descri = object.getString("Descri_Servi");
                        String date = object.getString("Data_Previ");
                        String dateconc = object.getString("Data_Conclusao");

                        String servrel = object.getString("Servi_realizado");
                        String tecresp = object.getString("Tec_Resp");
                        int valorp = object.getInt("Vlr_Peca");
                        int valorm = object.getInt("Vlr_MO");
                        int valort = object.getInt("Vlr_Total");
                        String status = object.getString("Status");
                        String comp = String.valueOf(object.getInt("Id_Compensa"));

                        nid = String.valueOf(id1);
                        nnome = name;
                        nidcliocor = idclienteocor;
                        neditDescri = descri;
                        nDataPrev = date;
                        nDataConc = dateconc;
                        neditServRealizado = servrel;
                        neditTec = tecresp;
                        neditValorP = String.valueOf(valorp) ;
                        neditValorM = String.valueOf(valorm) ;
                        neditValorTotal = String.valueOf(valort);
                        neditStatus = status;
                        ncomp = comp;
