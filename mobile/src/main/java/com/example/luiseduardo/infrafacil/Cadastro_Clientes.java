...
// AppCompatActivity {
    ...
    private static String GETINFO_URL = "http://futsexta.16mb.com/Poker/insert_clienteMobile.php";
            ...
            if (json != null) {
                try {
                    ...
                    for (int i = 0; i < eventDetails.length(); i++) {
                        ...
                        String descri = c.getString("Descri_Servi");
                        ...
                        if (status.equals("Executando")) {
                            ItemListView item1 = new ItemListView(name,descri,id, R.mipmap.trabalho100);

                            itens.add(item1);
                        }

                        if (status.equals("Concluido")) {
                            ItemListView item2 = new ItemListView(name,descri,id, R.drawable.checkmark);

                            itens.add(item2);
                        }
                        if (status.equals("Aguardando Compra")) {
                            ItemListView item3 = new ItemListView(name,descri,id, R.drawable.shopping128);

                            itens.add(item3);
                        }
                        if (status.equals("Aguardando Pag.")) {
                            ItemListView item4 = new ItemListView(name, descri, id, R.mipmap.money100);

                            itens.add(item4);
                        }
                        if (status.equals("Aguardando Retirada")) {
                            ItemListView item5 = new ItemListView(name, descri, id, R.mipmap.boxout128);

                            itens.add(item5);
                        }
                        if (status.equals("Orçamento não aprovado")) {
                            ItemListView item6 = new ItemListView(name, descri, id, R.mipmap.unlike);

                            itens.add(item6);
                        }
                        if (status.equals("Cancelado")) {
                            ItemListView item7 = new ItemListView(name, descri, id, R.mipmap.camcel);
        ...
        protected void onPostExecute(Void result) {
            ...
            if (pDialog.isShowing()) {
             ...
             if (json.isEmpty()) {
                ...
                itens = new ArrayList<ItemListView>();
                newItemlist.clear();
                ItemListView item7 = new ItemListView("0", "Cliente não possue Chamados", "0", android.R.drawable.presence_busy);
                itens.add(item7);
